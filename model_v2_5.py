...
def train_model(model, train_data, valid_data, num_epochs=250, batch_size=32, max_grad_norm=1.0, patience=20,
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_loss:
            best_loss = val_loss
            save_model(model, "best_stock_binary_optim_lstm_model.pth")
            epochs_no_improve = 0
...
def objective(trial, processed_data):
    ...
    Args:
        ...
        processed_data (dict): Preprocessed data containing train, valid, and test splits.

    Returns:
        float: Validation loss to minimize.
    """
    # Suggest hyperparameters
    num_layers = trial.suggest_int('num_layers', 1, 4)  # e.g., 1 to 4 layers
    batch_size = trial.suggest_categorical('batch_size', [16, 32, 64, 128])
    hidden_size = trial.suggest_categorical('hidden_size', [50, 64, 75, 85, 100])

    # Fixed hyperparameters
    input_size = 5
    # hidden_size = 64  # You might also consider optimizing this
    dropout_rate = 0.5
    learning_rate = 0.001
    num_epochs = 145  # Reduce number of epochs for faster optimization
    patience = 20  # Adjust accordingly

    # Initialize model with the sampled num_layers
    ...
    # Train the model with the sampled batch_size
    ...
    return best_val_loss  # Optuna tries to minimize this value
...
if __name__ == "__main__":
    # Initialize parameters (only ones not being optimized)
    ...
    study = optuna.create_study(direction='minimize')  # We aim to minimize validation loss
    study.optimize(lambda trial: objective(trial, processed_data), n_trials=n_trials)
    ...
    # After finding the best hyperparameters, retrain the model on the full training set and evaluate on test set
    best_num_layers = trial.params['num_layers']
    best_batch_size = trial.params['batch_size']

    print("\nRetraining the model with the best hyperparameters...")

    # Initialize the model with best_num_layers
    input_size = 5
    hidden_size = 64
    dropout_rate = 0.5
    categorical_size = processed_data['train']['cat'].shape[1]
    model = StockLSTM(input_size, hidden_size, best_num_layers, categorical_size, dropout=dropout_rate).to(device)
    ...
    # Load the best model
    best_model = load_model("best_stock_binary_optim_lstm_model.pth", device)
    ...
    save_model(best_model, "best_stock_binary_optim_lstm_model_final.pth")